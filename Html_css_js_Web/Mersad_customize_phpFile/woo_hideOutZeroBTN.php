...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
        ...
        if ( $product->is_type( 'simple' ) && ( $product->get_price() == 0 || $product->get_price() === '' ) ) {
            ...
            if ( $link ) {
                ...
                add_action( 'woocommerce_single_product_summary', function() use ( $link ) {
                    echo '<a href="' . esc_url( $link ) . '" class="mh-view-button">مشاهده محصول</a>';
                }, 30 );
            }
        }
    }, 5 );
    ...
    add_filter( 'woocommerce_loop_add_to_cart_link', function( $button, $product ) {
        ...
        if ( $product->is_type( 'simple' ) && ( $product->get_price() == 0 || $product->get_price() === '' ) ) {
            ...
            if ( $link ) {
                return '<a href="' . esc_url( $link ) . '" class="mh-view-button">مشاهده</a>';
            }
        }
        return $button;
    }, 10, 2 );
    ...
    add_filter( 'woocommerce_cart_item_name', function( $name, $cart_item, $cart_item_key ) {
        ...
        if ( $product->is_type( 'simple' ) && ( $product->get_price() == 0 || $product->get_price() === '' ) ) {
            $link = get_post_meta( $product->get_id(), '_mh_hashed_link', true );
            if ( $link ) {
                return $name . '<br><a href="' . esc_url( $link ) . '" class="mh-view-button">مشاهده محصول</a>';
            }
        }
        return $name;
    }, 10, 3 );
    ...
    add_filter( 'woocommerce_add_to_cart_validation', function( $valid, $product_id, $quantity ) {
        ...
        if ( $product->is_type( 'simple' ) && ( $product->get_price() == 0 || $product->get_price() === '' ) ) {
            ...
            if ( $link ) {
                ...
                wc_add_notice( 'این محصول رایگان است. برای مشاهده روی دکمه "مشاهده محصول" کلیک کنید.', 'error' );
                return false;
            }
        }
        return $valid;
